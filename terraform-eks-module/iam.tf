...
resource "aws_iam_role" "eks_master_role" {
  ...
  name = join("-", ["role-cluster", local.name, random_id.id.hex])
...
resource "aws_iam_policy" "ALBIngressControllerIAMPolicy" {
  name   = join("-", ["policy-IngressController", local.name, random_id.id.hex])
  policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role" "eks_alb_ingress_controller" {
  name        = join("-", ["role-ingress-controller", local.name, random_id.id.hex])
...
resource "aws_iam_role" "eks_node_role" {
  ...
  name = join("-", ["role-node", var.environment, each.key, random_id.id.hex])
...
resource "aws_iam_policy" "external_dns" {
  name   = join("-", ["external-dns", local.name, random_id.id.hex])
  policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
